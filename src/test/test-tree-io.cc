...
struct TestTreeIO {
    ...
    TestTreeIO() {
        ...
        // take care that the space-char at the tail is important for string-based comparison.
        graph_str =
    "{\"nodes\": ["
        "{\"id\": 0, \"sym\": \"ROOT\", \"span\": [0, 2], \"trg_span\": [0, 2]}, "
        "{\"id\": 1, \"sym\": \"VP\", \"span\": [0, 2]}, "
        "{\"id\": 2, \"sym\": \"NP\", \"span\": [0, 2]}, "
        "{\"id\": 3, \"sym\": \"JJ\", \"span\": [0, 1]}, "
        "{\"id\": 4, \"sym\": \"VP\", \"span\": [0, 1]}, "
        "{\"id\": 5, \"sym\": \"VPG\", \"span\": [0, 1]}, "
        "{\"id\": 6, \"sym\": \"running\", \"span\": [0, 1]}, "
        "{\"id\": 7, \"sym\": \"NP\", \"span\": [1, 2]}, "
        "{\"id\": 8, \"sym\": \"NN\", \"span\": [1, 2]}, "
        "{\"id\": 9, \"sym\": \"water\", \"span\": [1, 2]}"
    "], \"edges\": ["
        "{\"id\": 0, \"head\": 0, \"tails\": [1], \"features\": {\"parse\": 1.5}, \"score\": 1.5}, "
        "{\"id\": 1, \"head\": 0, \"tails\": [2]}, "
        "{\"id\": 2, \"head\": 1, \"tails\": [4, 7]}, "
        "{\"id\": 3, \"head\": 2, \"tails\": [3, 8]}, "
        "{\"id\": 4, \"head\": 3, \"tails\": [6]}, "
        "{\"id\": 5, \"head\": 4, \"tails\": [5]}, "
        "{\"id\": 6, \"head\": 5, \"tails\": [6]}, "
        "{\"id\": 7, \"head\": 7, \"tails\": [8]}, "
        "{\"id\": 8, \"head\": 8, \"tails\": [9]}, "
        "{\"id\": 9, \"head\": 8, \"trg\": [{\"label\": -1, \"words\": [\"\\\\\\\"\", -1]}]}"
    "], \"words\": ["
        "\"running\", "
        "\"water\""
    "]}";
...
// ****** The tests *******
BOOST_FIXTURE_TEST_SUITE(tree_io, TestTreeIO)

BOOST_AUTO_TEST_CASE(TestReadPenn) {
    // Use this tree_str
    ...
    BOOST_CHECK(tree_exp.CheckEqual(*hg_act) && left_act == left_exp);
}

BOOST_AUTO_TEST_CASE(TestReadPennEmpty) {
    PennTreeIO io;
    HyperGraph hg_exp;
    // Two types of empty trees 
    istringstream instr1("()");
    boost::scoped_ptr<HyperGraph> hg_act1(io.ReadTree(instr1));
    BOOST_CHECK(hg_exp.CheckEqual(*hg_act1));
    // // Check that both values are equal
    // istringstream instr2("(())");
    // boost::scoped_ptr<HyperGraph> hg_act2(io.ReadTree(instr2));
    // BOOST_CHECK(hg_exp.CheckEqual(*hg_act1) && hg_exp.CheckEqual(*hg_act2));
}

BOOST_AUTO_TEST_CASE(TestReadRule) {
    // Use this rule_str
    istringstream instr(rule_str);
    RuleTreeIO io;
    boost::scoped_ptr<HyperGraph> hg_act(io.ReadTree(instr));
    // Check that both values are equal
    BOOST_CHECK(tree_exp.CheckEqual(*hg_act));
}

BOOST_AUTO_TEST_CASE(TestReadEgret) {
    // Use this tree
    istringstream instr(egret_str);
    EgretTreeIO io; io.SetNormalize(false);
    boost::scoped_ptr<HyperGraph> hg_act(io.ReadTree(instr));
    // Check that both values are equal
    BOOST_CHECK(egret_exp.CheckEqual(*hg_act));
}

BOOST_AUTO_TEST_CASE(TestWriteEgret) {
    // Use this tree
    stringstream strm;
    EgretTreeIO io; io.SetNormalize(false);
    PennTreeIO pio;
    ...
    // Check that both values are equal
    ostringstream oss_exp; pio.WriteTree(tree_exp, oss_exp);
    ostringstream oss_act; pio.WriteTree(*hg_act, oss_act);
    BOOST_CHECK(CheckEqual(oss_exp.str(), oss_act.str()));
}

BOOST_AUTO_TEST_CASE(TestReadJSON) {
    // Use this tree
    ...
    string left_exp = "AAA";
    // Check that both values are equal
    BOOST_CHECK(graph_exp.CheckEqual(*hg_act) && left_act == left_exp);
}

BOOST_AUTO_TEST_CASE(TestWriteJSON) {
    // Use this tree
    stringstream strm;
    JSONTreeIO io;
    ...
    // Check that both values are equal
    BOOST_CHECK(CheckEqual(graph_str, str_act));
}

BOOST_AUTO_TEST_CASE(TestRoundtripJSON) {
    // Use this tree
    stringstream strm;
    JSONTreeIO io;
    io.WriteTree(graph_exp, strm);
    boost::scoped_ptr<HyperGraph> hg_act(io.ReadTree(strm));
    // Check that both values are equal
    BOOST_CHECK(graph_exp.CheckEqual(*hg_act));
}

BOOST_AUTO_TEST_CASE(TestRoundtripJSONQuote) {
    // Use this tree
    stringstream strm;
    JSONTreeIO io;
    ...
    boost::scoped_ptr<HyperGraph> hg_act(io.ReadTree(strm));
    // Check that both values are equal
    BOOST_CHECK(quote_exp.CheckEqual(*hg_act));
}

BOOST_AUTO_TEST_CASE(TestWritePenn) {
    ...
    BOOST_CHECK(CheckEqual(tree_str, act_str));
}

BOOST_AUTO_TEST_CASE(TestWriteRule) {
    RuleTreeIO rule;
    istringstream iss(rule_str);
    boost::shared_ptr<HyperGraph> graph(rule.ReadTree(iss));
    ostringstream oss; rule.WriteTree(*graph, oss);
    string act_str = oss.str();
    BOOST_CHECK(CheckEqual(rule_str, act_str));
}

BOOST_AUTO_TEST_CASE(TestWriteMosesXML) {
    string tree_str = "(A (B (C x) (D y)) (E z))";
...
BOOST_AUTO_TEST_CASE(TestReadWord) {
