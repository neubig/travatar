...
// ****** The tests *******
BOOST_FIXTURE_TEST_SUITE(hyper_graph, TestHyperGraph)

BOOST_AUTO_TEST_CASE(TestCalculateSpan) {
...
BOOST_AUTO_TEST_CASE(TestInsideOutside) {
    ...
    BOOST_CHECK(CheckAlmostVector(prob_exp, prob_act));
}

BOOST_AUTO_TEST_CASE(TestInsideOutsideUnbalanced) {
...
BOOST_AUTO_TEST_CASE(TestCalculateSpanForest) {
...
BOOST_AUTO_TEST_CASE(TestCalculateFrontier) {
    vector<set<int> > src_span = align1.GetSrcAlignments();
    ...
    BOOST_CHECK(CheckVector(exp, act));
}

BOOST_AUTO_TEST_CASE(TestCalculateNull) {
...
BOOST_AUTO_TEST_CASE(TestCalculateFrontierForest) {
    vector<set<int> > src_span = align2.GetSrcAlignments();
    vector<set<int>*> node_span(src2_graph->NumNodes(), (set<int>*)NULL);
    // Here, we will treat terminal nodes as non-frontier, as we just
...
BOOST_AUTO_TEST_CASE(TestNbestPath) {
    ...
    BOOST_CHECK(CheckPtrVector(exp_nbest, act_nbest));
}
...
BOOST_AUTO_TEST_CASE(TestNbestTied) {
...
BOOST_AUTO_TEST_CASE(TestNbestUniq) {
    // Accumulate Viterbi scores over nodes
    ...
    BOOST_CHECK(CheckPtrVector(exp_nonu, act_nonu) && CheckPtrVector(exp_uniq, act_uniq));
}

BOOST_AUTO_TEST_CASE(TestPathTranslation) {
    // Get the three-best edge values
    ...
    BOOST_CHECK(CheckVector(exp_trans, act_trans));
}
...
BOOST_AUTO_TEST_CASE(TestAppend) {
    ...
    BOOST_CHECK(hgab_exp.CheckEqual(hga));
}

// See if labeling spans works
BOOST_AUTO_TEST_CASE(TestGetLabeledSpans) {
    std::string src1_tree = "(S (NP (PRP he)) (VP (AUX does) (RB not) (VB go)))";
    ...
    BOOST_CHECK(CheckMap(exp_spans, act_spans));
}
