...
public:
    ...
    int TestReadPenn() {
        ...
        // Make the edge for A
        HyperEdge * a_edge = new HyperEdge(hg_exp.GetNode(0));
        a_edge->AddTail(hg_exp.GetNode(1)); a_edge->AddTail(hg_exp.GetNode(4));
        a_node->AddEdge(a_edge); hg_exp.AddEdge(a_edge);
        // Make the edge for B
        HyperEdge * b_edge = new HyperEdge(hg_exp.GetNode(1));
        b_edge->AddTail(hg_exp.GetNode(2)); b_edge->AddTail(hg_exp.GetNode(3));
        b_node->AddEdge(b_edge); hg_exp.AddEdge(b_edge);
        // Make the edge for C
        HyperEdge * c_edge = new HyperEdge(hg_exp.GetNode(2));
        c_node->AddEdge(c_edge); hg_exp.AddEdge(c_edge);
        // Make the edge for D
        HyperEdge * d_edge = new HyperEdge(hg_exp.GetNode(3));
        d_node->AddEdge(d_edge); hg_exp.AddEdge(d_edge);
        // Make the edge for E
        HyperEdge * e_edge = new HyperEdge(hg_exp.GetNode(4));
